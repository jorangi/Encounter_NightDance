...
### 2026-03-18
조금은 변화가 있다. 일단 UnitStat에 대해 고민이 많았는데, 기존에는 VitalUnitStat이 있고, 정신력이 더해지면 UnitStat이 있었다.
고민되는 부분이 바로 그 부분이다. 어떤 유닛은 체력만 있고, 어떤 애는 정신력만 있고, 둘 다 있는 애도 있을 것이고.
심지어는 장비를 착용하는 인간 유닛, 괴물 유닛은 장비같은게 있으면 안될 것 같고.
그러다보면 VitalUnit, MentalUnit, HybridUnit, withEquipUnit 등등 난장판이 될게 눈에 뻔히 보였다.
어떻게 하는게 좋을까 생각을 해봤는데, 빌더 패턴을 사용해서 여러 기능들을 피킹하면 되지 않을까 했는데, 각 기능들을 필드로서 남기는게 너무 지저분할 것 같았다.
...
사실 이 외에도 조금씩 바뀐 부분이 있을텐데 몰입하느라 Reademe에 기록하지 못했다.
지웠다 썼다도 여러번 하다보니 git에 반영되지 못하는 부분도 있긴 하다.
중간에 스택오버플로가 있었는데, Stat의 Calc에서 이벤트를 Invoker하는 부분이 있는데, 외부에서 이벤트에 등록된 함수가 Stat.Value를 가져다 쓰는 부분으로 인해 발생하는 것을 알 수 있었다.
Calc는 이중으로 보험처럼 넣었는데, 애시당초 설계를 좀 깐깐하게 해서 이벤트를 Calc가 아니라 Calc를 유도하는 부분에 이미 이벤트 Invoke가 있어서 그냥 지웠다.

원래는 UI를 연결하고자 했는데, VitalUnitStat, UnitStat:VitalUnitStat 이라는 구조가 너무도 거슬려서 참을 수 없었고 이를 해결하는데 생각보다 시간이 걸렸다.
그만큼 설계를 처음부터 잘 하는 것이 중요하기는 하지만, IUnitCore부분을 보면 알 수 있듯이 이중으로 상호연결된 부분을 볼 수 있다.
상호참조 부분에 있어서는 정말 조심스럽게 처리 했지만, 원칙을 완전히 지키지는 못하지만 인터페이스 분리를 통해 상호참조 문제는 해결하고 실전성과 생산성을 챙겼다.
...
### 2026-03-24
지난날에 했던 작업을 실제 UI와 연동하는 작업을 진행하였다.
유니티 UI 디자인패턴으로 MVC, MVP, MVVM 등 여러가지가 있는데 찾아보면 R3라는 라이브러리도 있다고 한다.
다만 R3는 지금 도입하기에는 학습 곡선이 높기에 MVP 패턴을 도입할 생각이다. 다만 아직은 진행하지는 않고 합쳐놓은 Smart UI 디자인패턴에 가깝다.
UI의 계층 분리가 제대로 되지 않은 상태이기 때문이다.
View가 Model에 너무 깊이까지 파헤치고 있기 때문이다.
아마 다음 커밋 때 Model, View, Presenter의 계층 분리를 진행할 예정이다.
