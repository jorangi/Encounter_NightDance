...
### ~2026-07-06
다른 프로젝트를 하느라 한동안 뜸했던 것 같다. 프로젝트의 에디터 버전을 변경하였다.<br>
보안 문제가 첫 번째 원인이었고, 그래서 바꾼 버전은 인풋 액션의 문제가 있었다.<br>
인풋 액션에 액션맵을 추가해도 정상적으로 적용되지 않고 reimport하면 초기화되는 문제였다.<br>
나만 그런것이 아니라 유니티 토론장에서도 등장했다.<br>
일단은 버전을 다시 한 번 Recommand로 변경하였고 여전히 적용은 바로 안되지만 액션맵의 세팅에서 저장을 직접 할 수 있게되었다.<br>
이제부터 한동안은 다시 이 프로젝트에 집중할 예정이다.

### 2026-07-08
- 임시로 두 개의 UI를 배치하였다.
- InputAction의 Action Map을 추가하였다. (UnitControl)
- 포커스의 위치가 변할 경우 `Router Rednerer`가 최대한 사용자의 지정한 경로를 따라 렌더링 되도록 하였다.
    - 그려진 경로가 여전히 최단 경로인 경우에만 유지되며, 그것이 아니라면 초기화하고 최단 경로를 렌더링한다.
- 유닛에게 `Schedule`시스템을 추가하여 지정한 이동 경로(혹은 대상)로 향한다.
    - 다음 턴에도 해당 위치가 유지되며, 가령 10칸 너머의 위치에 가기를 원하지는 이동력이 3칸인 경우 설정한 위치를 저장해두고 다음 턴에도 그 경로를 따라 움직인다.
    - 현재 저장하는 데이터는 단순 Vector2Int이기에, Vector2Int Arr 등을 통해 경로 자체를 저장하게끔 해야할 것이다.

### 2026-07-11
- 젠킨스 빌드 테스트 중이다.
    - BuildScript를 작성하여 젠킨스와 연동시켰다.
    - Git Webhook을 이용해 푸시 알림을 보내 젠킨스에게 자동 빌드를 하게끔 했다.

- 유닛 생성을 위해 Factory를 사용할까 생각중이다.
- VContainer(DI)를 사용해보고자 한다.
    - 유닛을 팩토리처럼 생성하고 DI하기 위해 UnitManager를 일단 추가하는 중이다.

### 2026-07-13
- `MultiKeyMap` 자료구조 클래스를 추가했다.
    - 바로 레거시화되긴 했지만 원래계획은 Key를 자유롭게 N개 받아서 사용할 수 있게 하는 것이었다.
    - 그런데 문제는 object를 사용하게 되면 value-type의 박싱 현상이 일어나게 되면서 성능 저하가 유발된다는 것이었다.
    - 사용하고자 했던 곳이 유닛의 id와 pos각각의 키로 Unit을 찾는게 목적이었기에 그냥 일단은 2개의 키만을 제네릭으로 허용시켰다.

- `IFactionFeature` 추가
    - 유닛들에게 팩션이 지정될 예정이다.
    - 현재는 땜빵용으로 Player, Enemy, Friendly, None으로 구분해놓긴 했는데, 원래는 세력으로 분류할 생각이다.
    - 적들끼리도 세력이 나뉘어있게끔 할 예정이다.
    - 해당 인터페이스는 IUnitFeature를 상속받기 때문에, 팩션 Feature를 갖게될 때, 잃게될 때 등의 구현이 가능하다.

- `PathFinder` 최적화
    깃 커밋을 자주 안해서 올린 줄 알고있었던 내용이다.
    - PathFinder의 함수 내부 필드들을 private static으로 외부에 선언하여 사용하게끔 하였다.
    - openList를 List에서 minheap으로 변경하여 O(log N)으로 개선하였다.
    - closeList를 List에서 HashSet으로 변경하여 O(1)으로 개선하였다.
    - PathFinderNode를 클래스에서 구조체로 변경하며 내부에 소유하던 Parent 객체를 단순히 근거지 노드의 좌표로 표시하게끔 변경하였다.
        - 따라서 이에 대한 보강으로 자신이 들어온 방향을 저장하게 하였고, 이는 노드를 구분하기 위한 척도로서 좌표, 방향성이 사용되는 원인이 되었다.
    - 이 개선을 통해 성능이 소폭 향상되었겠지만 UnitFactory를 만들면서 테스트 환경이 깨져 지금 당장은 비교하기 어렵다.
        - 로직의 이용 자체는 동일하니 나중에 테스트가 가능할 것이다.

- `UnitFactory` 추가
    - 유닛을 생성하는 팩토리를 추가했다.
        - 본래는 정적 팩토리 메서드를 활용해서 만들까 했는데, `VContainer`도 써볼 겸 DI를 사용했다.
        - 정확히는 `UnitFactory`는 주입하는 쪽이고 `UnitManager`가 주입받는 쪽이다.

- `UnitManager` 추가
    - 유닛의 생명주기만을 담당하게 하기 위해 만든 클래스이다.
    - Unit을 스폰하고, 제거하고 탐색하는 정도의 역할을 구현했다.
    - DI를 통해 UnitFactory를 Singletone 라이프타임 스코프로 주입받아 사용한다.
    - 유닛 매니저는 딱히 다른 오브젝트에 종속되게끔 하는게 아니라 `Event Driven Architecture`를 이용할 생각이라서<br>
        이벤트를 통해 외부에서 '유닛을 스폰해주세요' 혹은 '유닛을 제거해주세요' 등의 명령을 전달받고 처리할 수 있도록 설계하고자 한다.

이외에 이벤트 주도 아키텍쳐를 위해 그냥 Action이든 R3만 써도 상관없긴 한데, 새로운 MessagePipe라는 라이브러리를 써보고자 한다.<br>

### 2026-07-15
14일에 작업하고 깃허브 올리는 걸 까먹었다..<br>
작업한 내용이 꽤 있다.<br>

- DOTween 에셋 추가
    아직 실제 사용은 안했는데, 정적인 스프라이트를 활용하여 최저한의 비용으로 만들다 보니 다소 밋밋해보일 수 있겠다고 생각하여 추가했다.<br>
    DOTween은 유료 에셋이라고 생각했는데, 제공되는 UI를 제외하고 스크립트만을 활용하여 개발하는 경우는 무료로 사용이 가능한 버전이 있길래 그걸로 했다.

- SpriteWithShadowSmooth 쉐이더 추기
    조금 뜬금없긴 한데, 기존에 SpriteWithShadow 쉐이더는 이미 있었고, 그걸 이용했다.<br>
    필드에 배치된 유닛 스프라이트의 아래가 그냥 밋밋하게 있는 것 보다는 투명~불투명 그라데이션 처리를 하면 시각적으로 조금 더 완성도가 높아보이지 않을까 해서 작업했다<br>

본격적으로 유닛을 스크립트 기반으로 배치하기 위한 작업을 시작했다.

- 일단은 테스트용으로 사용하던 유닛 오브젝트를 프리팹화 하였다.
- 생성 플로우는
    1. 유닛 생성 요청 이벤트 발행
    2. 유닛 매니저에서 이벤트를 받아 커맨드패턴으로 유닛 생성 커맨드를 실행
    3. 유닛 생성 커맨드에서 유닛 팩토리를 통해 유닛을 생성
    이런 흐름이다.<br>
    굳이 커맨드만 쓰면 되는데 이벤트를 왜 넣었냐는 의문도 들긴 했는데, 사실 그 부분에서 고민을 많이 했다.<br>
    지난 번에 하던 The Tower Of Trials 프로젝트를 하면서 이벤트 버스를 만들고 이벤트 기반으로 굉장히 많은 부분을 처리했는데,<br>
    `Event Driven Architecture`가 굉장히 매력적이고 성능도 좋았다.<br>
        그래서 이번에는 본격적으로 CySharp(CyGames)의 오픈소스인 `MessagePipe` 라이브러리를 통해 `Event Driven Architecture`를 구현하고자 했다.<br>
        기존에 사용하던 R3와 호환성이 좋음을 확인했고, 같은 곳에서 만든 `UniTask`도 호환성이 좋다고 했다.<br>
        `VContainer는` 다른 개발자가 만들었지만 같이 프로젝트에 참여해서 만들었기에 좋다고 하더라.<br>
        이외에 `MemoryPack`도 괜찮아 보인다. 기존의 JSON Serializer에서 구조체 기반으로 새로 만든 직렬화 관련 라이브러리인데, 쓸 일이 많아 요긴해보인다.<br>
        Stars 순으로 보면 `UniTask`, `ZLinq`, `MemoryPack`, `MagicOnion`, `R3`, `ZString`,... 되게 많다😅
    다시 본론으로 돌아와서 이벤트 기반으로 아키텍쳐를 구상하고, 가능한 일관성을 깨고 싶지 않아서 이벤트 -> 커맨드의 흐름을 따르게 됐다고 볼 수 있다.

- `BattleCommandFactory` 추가
    - 아직까지는 `MoveCommand`만 있다.
    - 전투 관련 로직들을 이곳에서 생성하게 된다.
    - 보일러 플레이트인가 싶기는 하지만, 커맨드를 생성하는 주체에서 전부 이벤트 퍼블리셔를 가져야하는 게 맘에 들지 않아서 카테고리별로 묶게끔 했다.
- `UnitCommandFactory` 추기
    - 유닛의 소환이나 해체 커맨드 등을 생성하게 된다. (현재는 생성만 있다.)
    - 위와 마찬가지로 일관성을 위해 만들었다.
- `MoveCommand` 변경점
    - 생성자에 path와 publisher가 추가로 주입된다.
    - `Execute`부분에 이동처리에 관한 이벤트가 추가되었다.
        - 정확히는 이동을 하였다라고 알리는 이벤트로, 나중에 이동 거리에 따라 피해를 입는다거나 등의 효과를 위해 추가했다.
        - 들어가는 페이로드의 값은 이동한 거리, 시작점, 도착점이다.
    - 이전에 구현한 `ProcessUnitMoveSchedule`은 유닛에게 목표하는 위치를 알려주고, 유닛의 이동력이 이를 바탕으로 `MoveCommand`를 작성하여 실행하기에 페이로드에 들어가는 것은 현재 이동한 곳만을 담는다.
    - 이와 보강하여 현재 유닛이 목표한 곳까지 얼마나 이동해왔는지, 얼마나 남았는지, 시작지점부터 목표지점까지의 거리는 원래 몇인지를 저장하는 변수를 추가하였다.
        - `MoveCommand`는 여기서 얼마나 이동해왔는지에 영향을 주게된다.
- `UnitSpawnCommand` 추가
    - 유닛 생성을 위한 커맨드를 추가했다.
    - 앞서 말한 것과 같이 이벤트를 통해 `UnitManager`가 `UnitCommandFactory`를 통해 호출한다.
    - 작동 내용은 다음과 같아.
        1. `UnitManager`는 `ReactiveProperty<Unit>`을 커맨드로 주입한다.
        2. 커맨드는 주입받은 `ReactiveProperty<Unit>`를 `UnitFactory`로 주입하며 Create한다.
        3. `UnitFactory`는 Create하여 실제 게임오브젝트로서 유닛을 생성하고 이를 주입받은 `ReactiveProperty<Unit>`에 저장한다.
        4. `UniSpawnCommand`가 돌려받은 `ReactiveProperty<Unit>`은 원래 `UnitManager`의 것이므로 그대로 돌아간다.
        5. `UnitManager`는 생성된 유닛을 필드에 소환하고(좌표를 맞춤) 그 `EntityId`를 반환한다.
            - 원래 Id는 `GetInstanceId`를 사용할 예정이었는데, 유니티에서 `DOTS`를 밀어주느라고 `ECS`의 일원으로 `EntityId`로 대체되었다.
            - 그래서 `EntityId`를 사용하게 되었다.
- `MoveRequestPayload` 추가
    - 유닛이 이동을 요청할 때 사용하는 페이로드이다.
    - 내용은 간단하다.
        - 누가 이동하는가
        - 어디로 이동하는가
        - 경로는 어떻게 되는가
    - 경로를 페이로드에 집어넣은 것은 `ProcessUnitMoveSchedule`이 원인이기는 한데, 쓰임새는 다양할 것 같다.
- `UnitSpawnPayload` 추가
    - 유닛을 소환할 때(생성+소환) 사용하는 페이로드이다.
    - 내용은 유닛의 데이터와 소환하는 포지션 좌표 뿐이다.
    - 고민인 부분은, 유닛의 데이터를 생성하는 것과 소환하는 것은 서로 다른 작업이므로 분리해야 한다는 생각이 있기는 하다.
        - 가령 전투 준비 단계에서 유닛을 얻었는데 이는 필드가 아니니 소환이 의미없다는 뜻이다.
        - 아마 변경을 최소화 한다면 유닛을 게임상에 추가하는 이벤트를 별도로 추가하고 이 이벤트는 추가된 유닛을 소환하는 용도로만 쓰는 것이다.
- `CommandInvoker` 변경점
    - 기존에 있던 다른 코드들에게도 있던 추가점을 까먹었는데, `IStartable` 인터페이스가 일부 클래스에 추가되었다.
        - `VContainer`를 사용하며 특정 객체가 생성하여 보관하는게 아니라 DI로 주입하기에 시작시점에 미리 생성되어 있어야 하기 때문이다.
    - `ExecuteCommand`의 함수가 bool 값을 반환하도록 하였다.
        - `CanExecute`의 발생 주체는 커맨드다보니, `CommandInvoker`에서도 실행 성공 여부를 반환해야지 맞지 않나 생각이 들어서 변경하게 되었다.
    - 함수 summary 주석을 추가했다.

- `FieldManager` 의 UnitPrefab을 제거했다. 원래 쓰이지도 않았는데 아마 넣어놓고 까먹었던것 같다.
- `GameLifetimeScope` 변경점
    - 기존에는 `UnitFactory`와 `CommandInvoker`만 주입했는데, 이제는 주입하는 내용이 꽤 많아졌다.
    - 유닛 프리팹은 여기서 받고 필요로 하는 곳에 주입된다. ex) UnitManager
    - MessagePipe는 여기에서 만들어진다.
        - 즉, 이벤트를 구독하거나 발행하는 쪽 모두 DI를 통해 이벤트 퍼블리셔를 주입받아야 한다.
        - 함수명에도 나와있듯이 브로커 역할이다.
    - 이외에 `IStartable`과 연계를 위해 `AsSelf`로 등록했고, 이를 연쇄적으로 깨우기 시작해야하는 `UnitManager`는 `EntryPoint`로 했다.
    - 게임 매니저는 다른 것들보다도 확실하게 하나만 있을 거라 하이어라키에서 찾아서 가져오게끔 했다.
- `Prototype_GameManager` 변경점
    - 필드 매니저가 제거되었다.
        - 기존에는 유닛 이동 커맨드에 필드 매니저를 주입해야했기에 있었다.
        - 이제는 이동도 이벤트로서 처리하기에 필드매니저는 제거할 수 있었다.
            - 느슨한 결합이라 좋다
    - 테스트용이긴 한데, SO객체인 `UnitData`를 받아서 이벤트 발행하여 유닛을 생성하게끔 했다.
    - 외부 DI용 생성체에 이벤트 퍼블리셔와 커맨드 팩토리를 추가했다.
        - 커맨드 팩토리는 테스트용이라 나중에 지울 수 있을 것으로 보인다.
    - `ProcessUnitMoveSchedules`에서 이제 Path를 커맨드에 전달해야 해서 `GetRange`로 추출해서 보낸다.
- `Prototype_TileObject` 변경점
    - 사실 얘보다는 `Destination`이 변경되었다.
    - 위에서 언급한대로 3개의 필드를 추가했다.
        - `RemainingDistance`, `totalDistance`,  `passedDistance`인데
        - Remaining은 그냥 total-passed이다.
- `Unit` 변경점
    - 기존에 외부호출인 `Initialize`에서 `MainAction`을 생성하던것을 `Awake`로 변경했다.
        - `Initialize`는 외부에서 호출하다보니 `OnEnable`이 먼저 불리는데, 이로인해 NRE가 발생했다.
    - 원래는 같은 게임오브젝트가 가진 `UnitStat` 컴포넌트에 `BaseStat`이라는 `UnitData`를 받아서 사용했는데,
        - 이제는 외부에서 Data를 주입하여 생성하니 그 부분을 이관시켰다.
    - 그 외에는 null처리를 삼항연산자 처리하던 걸 `??=`로 변경했다.
        - 사실 이거는 원래부터 하고싶었는데, VSCode에서는 계속 경고가 났다. 문제없이 처리되는 걸로 봐서 C# 버전이 바뀌어서 그런듯.
    - 유닛 포커스를 필드매니저였나에서 했는데, 이걸 우선은 옮겼다.
        - 유닛이 생성되고 포커스를 잡아야 UI가 문제가 없는데, 순서때문에 문제가 다소 있었다.
        - 나중에 이것만 바꾸면 될 것 같다.

- `UnitFactory` 변경점
    - 이전 버전에는 `Instantiate`하고 바로 `GetComponent`를 하려 했는데, 아마 몇년전에도 이런식으로는 안되는걸 확인했었다.
    - 그걸 다시 했을뿐이다..🙄

- `UnitManager` 변경점
    - commandInvoker, unitCommandFactory를 추가하고 주입받는다.
    - 퍼블리셔도 주입받는데, 이는 필드로서 저장되기 보다는 그냥 `SpawnUnit`을 구독하기 위함이다.
    - `Unit`에서 `ReactiveProperty<Unit>`으로 바뀌었기 때문에 Value를 쓴다는 것도 나름 변경점
    - 팩션은 아직 인터페이스로서 `Feature`만 남아있는데, 이러다보니 NRE가 떠서 일단 주석처리 해놨다.
        - 우선순위가 조금 밀린다.
    - `Object.Destroy`를 `UnitEngine`네임스페이스로 고정하여 충돌 오류를 방지하였다.
    - 이벤트 구독이 있다보니 `IDisposable`을 상속받아 Dispose로 이벤트를 구독 해제하도록 했다.
...
- 유닛의 정보를 표시하는 UI 레이아웃을 과감히 수정했다.
    - 최대한 간촐하게 바꾸되 디자인은 컨셉에 어울릴 수 있게끔 노력했다.
    - Photoshop으로 한땀한땀..
    - 아직 하이어라키의 UI 배치는 더럽다.
        - 일단 위치 잡는 용도로!
- 유닛의 스탠딩 일러스트를 변경 중이다.
    - 저 레거시 유닛을 그대로 쓸지는 모르겠지만, 아무 쓸 거 같으니까..
        - 예전에 그렸던 그림을 제미나이한테 주고 그 그림체로 레거시 유닛을 그려봐라 했는데, 적당히 잘 나왔다.
        - 그렇게 만들어진 AI 그림을 포토샵으로 후보정? 수정했다. AI티가 많이 나서 내 그림체에 맞게 어느정도 수정했다.
- 유닛의 생명력과 정신력에 단순 수치 말고 상태 텍스트를 추가했다.
    - TMPro의 머티리얼을 Create Preset하여 추가하였다.
    - 추가한 프리셋에 Glow를 주고, Global Volume을 통해 발광을 구현했다.
    - 다만 이게 게임 씬에서는 적용이 안되길래 찾아보니, 카메라에서 Post-Processor 처리를 안해서 그런 것이었다.
    - 이렇게 되면 불필요한 다른 오브젝트에도 전부 Bloom이 걸려서 카메라를 분리해야 했다.
        - UI 전용 카메라를 추가하였다.

이외에 조금 더 그럴싸한 게임처럼 보이게 할 방법을 구상 중이다.<br>
Gemini와 Veo로 유닛을 실제 스프라이트로 SD화 해서 실제 필드에 있다면 어떨지 프로토타입 영상을 만들어보고<br>
Comfy를 통해서 Pixel Sprite를 만들고, WAN + FLF2V로 움직이게 해본 다음에 로토스코핑처럼 프레임을 따서 덧 그려볼까 생각 중이다.<br>
애니메이션 안넣을 생각이었는데, Veo로 만든 영상을 보니 때깔이 천지차이다.

### 2026-07-31

UI디자인이나 쉐이더, 머티리얼을 좀 집중적으로 보고있다.
생각한 느낌이 안나서 등의 이유로 뒤엎기가 부지기수다.

- Glowing UI 레이어를 분리하여 UI용 Post Processing을 따로 적용하게끔 하였다.
- 생명력바의 디자인 레이아웃을 분리하였다.
- 생명력바의 끝쪽 외곽선에 강광을 추가하였으며 대각선으로 줄어들게 하기 위해 쉐이더를 이용하였다.
- 정신력바도 체력바와 비슷하게 작업중이다.
- 생명력바의 심장이 뛰는 효과와 심박수 느낌을 주기 위해 쉐이더를 이용하여 스텐실 효과를 주었다. 다만 온전히 쉐이더만으로는 해결하기 힘들고 DOTween을 일부 사용하였다.
    -   마스크를 먼저 만들고 One Minus 노드를 이용해서 스텐실을 만들었다.
- 정신력바는 뇌의 모양을 넣었고 정신력이 낮아지면 Glitch느낌을 주기 위해 색상 글리치와 비디오가 찢어지는 듯한 느낌을 주었다.
    - Simple Noise와 Posterize, UV Offset을 이용하였다.
- 각 바에 수치를 표현하는 부분과 상태(Stable, Unstable, Crisis)표시 부분에 Underlay를 추가하여 발광효과를 넣었다.
