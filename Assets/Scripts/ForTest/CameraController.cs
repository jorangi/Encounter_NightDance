...
namespace Encounter.NightDance.Core
{
    ...
    /// 카메라 컨트롤러
    /// </summary>
    public class CameraController : MonoBehaviour, IFieldObject
    {
        ...
        private readonly ReactiveProperty<Vector2Int> _onFocusPosChanged = new(Vector2Int.zero);
        public ReadOnlyReactiveProperty<Vector2Int> OnFocusPosChanged => _onFocusPosChanged;

        private void UpdateFocusPos(Vector2Int newPos)
        {
            if (focusPos != newPos)
            {
                focusPos = newPos;
                _onFocusPosChanged.Value = newPos;
            }
        }

        public Vector2 WorldPos
        {
            get => new(CoordinateUtility.GetWorldPos(focusPos).x, CoordinateUtility.GetWorldPos(focusPos).z);
            set => throw new NotSupportedException("CameraController의 WorldPos는 수정할 수 없습니다.");
        }
        public Vector2Int Pos
        {
            get => focusPos;
            set => UpdateFocusPos(value);
        }

        public Destination GetCurrentDestination => throw new NotImplementedException();
        ...
        void Update()
        {
            ...
            if (!Mathf.Approximately(moveDelta.magnitude, 0.0f))
            {
                ...
                Vector2Int currentLogicalPos = fieldManager.GetTilePos(focus.position);
                currentLogicalPos = FieldManager.ClampToField(currentLogicalPos.x, currentLogicalPos.y);
                UpdateFocusPos(currentLogicalPos);
            }
        }
        /// <summary>
        ...
        {
            throw new NotSupportedException("Camera Controller는 SetDestination(IFieldObject)을 지원하지 않습니다.");
        }
        public void SetDestination(Vector2Int pos)
        {
            throw new NotSupportedException("Camera Controller는 SetDestination(Vector2Int)을 지원하지 않습니다.");
        }
        public void ClearDestination()
        {
            throw new NotSupportedException("Camera Controller는 ClearDestination()을 지원하지 않습니다.");
        }
        public void RefreshFocus()
        {
            _onFocusPosChanged?.OnNext(_onFocusPosChanged.Value);
        }
    }
}